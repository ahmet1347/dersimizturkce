exports.handler = async function(event, context) {
  ...
  try {
    ...
    // API anahtarı Netlify Environment Variable'dan geliyor - kodda görünmez!
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            {
              type: 'text',
              text: 'Sen Türkçe, Matematik, Fen Bilimleri, Fizik, Kimya, Biyoloji, Tarih, Coğrafya ve tüm derslerde uzman bir LGS/TYT/AYT sınav koçusun. Görseldeki soruyu dikkatlice oku ve ders türünü belirle. Her şıkkı tek tek değerlendir. Cevabını şu formatta ver:\n✅ Doğru Cevap: [Şık]\n📝 Açıklama: [Neden doğru, adım adım çöz]\n❌ Diğer şıklar neden yanlış: [Kısaca açıkla]\nMatematikte işlemleri adım adım göster. Türkçede anlam analizini yap. Fende formül ve kavramları açıkla. Türkçe yanıtla.'
            }
          ]
        }]
      })
    ...
    if (veri.error) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: veri.error.message })
      };
    }
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: 'Sunucu hatası: ' + hata.message })
    };
  }
};
